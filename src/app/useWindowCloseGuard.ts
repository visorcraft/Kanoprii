...
export function useWindowCloseGuard({
  ...
  useEffect(() => {
    ...
    // they don't accumulate next to the user's source across runs. After the
    // v1.10.0 markdown -> PDF view, originalPath on a non-PDF-source session
    // points at the generated sibling (materialize_document_pdf wrote it
    // there). On close we delete those siblings; the user can keep a permanent
    // copy via Save As. Failures are best-effort/silent.
    const discardGeneratedSiblings = async () => {
      const tasks = sessionsRef.current
        .filter((s) => s.sourceKind !== 'pdf' && s.sourcePath && s.originalPath && s.originalPath !== s.sourcePath)
        .map((s) =>
          invoke('discard_document_pdf', { generated: s.originalPath, source: s.sourcePath }).catch(() => {}),
        );
